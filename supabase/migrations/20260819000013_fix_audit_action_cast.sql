-- Fix audit_action enum cast in save_expense (comprehensive fix for CASE expression) -- Fixes: column "action" is of type audit_action but expression is of type text
-- Phase 1.10: Optimistic concurrency on save_expense (patched cast) — expectedUpdatedAt (§5.4 / §7.6)
...
begin
  ...
  values (v_trip, auth.uid(), 'expense', v_exp, (case when v_existing.id is not null then 'update' else 'create' end)::public.audit_action,
    v_prev,
